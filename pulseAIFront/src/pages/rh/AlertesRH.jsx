...
import { AlertTriangle, Info, CheckCircle, XCircle, Bell, Archive } from 'lucide-react';
...
export default function AlertesGlobales() {
  const { notifications, markAsRead, archiveNotification, markAllAsRead } = useNotifications();
  ...
  return (
    <div className="animate-fade-in-up space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <button onClick={() => markAllAsRead('RH')} className="flex items-center gap-2 rounded-xl border border-brand-secondary/20 px-4 py-2.5 text-sm font-medium text-brand-secondary hover:bg-brand-light transition-colors">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-[11px] text-brand-secondary/50">{alert.department} · {alert.time}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <button onClick={() => markAsRead(alert.id)} className="grid h-7 w-7 place-items-center rounded-lg hover:bg-white text-brand-secondary/50 hover:text-brand-secondary transition-colors" title="Marquer lu"><CheckCircle size={14} /></button>
                      <button onClick={() => archiveNotification(alert.id)} className="grid h-7 w-7 place-items-center rounded-lg hover:bg-white text-brand-secondary/50 hover:text-brand-secondary transition-colors" title="Archiver"><Archive size={14} /></button>
                    </div>
                  </div>
                  {isExpanded && (
